...
class T88ble():
    ...
    def with_column(self, label, values):
        ...
        row_label = []
        for i in self.rows:
            rows_in_list.append(list(i))
        for j in self.column_labels:
            row_label.append(j)
        if label in row_label:
            label_num = row_label.index(label)
            for k in range(len(rows_in_list)):
                rows_in_list[k][label_num] = values[k]
        else:
            for l in range(len(rows_in_list)):
                rows_in_list[l].append(values[l])
            row_label.append(label)
        return T88ble(rows_in_list, row_label)
    ...
    def select(self, labels):
        ...
        for x in labels:
            if x in self.column_labels == True:
                row_label+=[x]
