...
def safe_sum(fun, seq, missing=0):
    ...
    def wrap(fun, x, summed):
        try:
            total_value = fun(x) + summed
        except:
            total_value = missing + summed
        return total_value
...
class Chaser(Player):
    ...
    def energy(self, time):
        ...
        """
        if time % 9 == 0:
            return self.base_energy - ((self.goals * self.energy_expended ) + 0.1 * self.energy_expended)
        else:
            return self.base_energy - (self.goals * self.energy_expended)
...
class Seeker(Player):
    ...
    def energy(self, time):
        ...
        550
        """
        return self.base_energy - (self.energy_expended * time)
...
class Keeper(Player):
    ...
    def energy(self, time):
        ...
        if time < 30:
            ...
            return self.base_energy - (time//15)*(self.energy_expended * 0.8)
